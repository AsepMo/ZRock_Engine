...
import zrock.application.engine.widget.callback.DragListener;
import zrock.application.engine.widget.callback.DragStateListener;
import zrock.application.engine.widget.transform.CompositeTransformation;
import zrock.application.engine.widget.transform.ElevationTransformation;
import zrock.application.engine.widget.transform.RootTransformation;
import zrock.application.engine.widget.transform.ScaleTransformation;
import zrock.application.engine.widget.transform.YTranslationTransformation;
import zrock.application.engine.widget.util.ActionBarToggleAdapter;
import zrock.application.engine.widget.util.DrawerListenerAdapter;
import zrock.application.engine.widget.util.HiddenMenuClickConsumer;
...
public class SlidingRootNavBuilder {
    ...
    private SlidingRootNavLayout createAndInitNewRoot(View oldRoot) {
        ...
        newRoot.setId(R.id.zrock_root_layout);
    ...
    private ViewGroup getContentView() {
        ...
        if (contentView.getChildCount() != 1) {
            throw new IllegalStateException(activity.getString(R.string.zrock_ex_bad_content_view));
        }
        return contentView;
    }
    ...
    private View getMenuViewFor(SlidingRootNavLayout parent) {
        if (menuView == null) {
            if (menuLayoutRes == 0) {
                throw new IllegalStateException(activity.getString(R.string.zrock_ex_no_menu_view));
            }
    ...
    protected void initToolbarMenuVisibilityToggle(final SlidingRootNavLayout sideNav, View drawer) {
        if (toolbar != null) {
            ...
            ActionBarDrawerToggle toggle = new ActionBarDrawerToggle(activity, dlAdapter, toolbar,
                    R.string.zrock_drawer_open,
                    R.string.zrock_drawer_close);
            toggle.syncState();
