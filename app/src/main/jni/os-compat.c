...
Java_zrock_application_engine_content_resolver_Os_nativeFstat(JNIEnv *env, jclass type, jint fileDescriptor) {
    struct stat sb;

    int rc = TEMP_FAILURE_RETRY(fstat(fileDescriptor, &sb));
    if (rc == -1) {
        int error = errno;
        jclass errnoExceptionClass = (*env)->FindClass(env, "zrock/application/engine/content/resolver/ErrnoException");
